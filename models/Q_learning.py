...
class Q_learning_agent:
    class Q_net(nn.Module):
        def __init__(self, state_size, n_frames_state, action_dim, hidden_size):
            super(Q_learning_agent.Q_net, self).__init__()
            ...
            for n_th_frame_net in range(n_frames_state):
                ...
                dense_input_size = 4*4*64#((cnn_state_size ** 2) * 8)
    ...
    def __init__(self, state_size,
        ...
        hidden_size = 512
        self.q_net = Q_learning_agent.Q_net(state_size, n_frames_state=self.n_frames_state, action_dim=2, hidden_size=hidden_size).to(device)
        self.q_net_target = Q_learning_agent.Q_net(state_size, n_frames_state=self.n_frames_state, action_dim=2, hidden_size=hidden_size).to(device)

        self.q_net_learning_rate = 0.005
        ...
        self.replay_buffer = Q_learning_agent.ExperienceReplay(max_buffer_size=65000)
    ...
    def get_actions(self, states, target_network=False, return_indexes=False):
        with torch.no_grad():
            if target_network is True:
                ...
                q_values = self.q_net_target(states).detach().cpu().numpy()
                self.q_net_target.train()
            else:
                self.q_net.eval()
                q_values = self.q_net.forward(states).detach().cpu().numpy()
                self.q_net.train()

        action_indexes = np.argmax(q_values, axis=1)
    ...
    def interact(self, states, states_identifiers, steps, epsilon):
        ...
        #unique, counts = np.unique(actions_float[:, 0], return_counts=True)
        #unique, counts = np.unique(actions_float[:, 1], return_counts=True)
        ...
        for tuple in tuples: # if several games in //.
            ...
            tuple["action"] = torch.tensor(self.action_to_index(tuple["action"]["input_x"], tuple["action"]["input_y"]))
        ...
        for sample in samples:
            ...
            """b = []
            for i in sample["state"].numpy():
                b.append(i)

            if list(steps.values())[0] > 10:
                cv2.imshow("test", np.concatenate(b))
                cv2.waitKey(0)
            """
    ...
    def train(self, n_episodes):
        ...
        while total_n_episodes < n_episodes:
            ...
            rewards, is_terminal, identifiers = self.interact(states, states_identifiers, steps, epsilon) # unsqueeze to make the images one chanel (instead of 0)
...
if __name__ == "__main__":
    ...
    agent = Q_learning_agent(state_size=64,
                             n_frames_state=1,
